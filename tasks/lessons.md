...
- **[pre-commit cache format]** v1 hook checks `cache.passed` (boolean), v2 writes `cache.result` (string). The pre-commit hook must be updated to match v2's VerifyCache format or commits will always be blocked.
