...
- **[skip teams for sequential work]** Don't spawn agent teams for milestones with purely sequential dependencies (each task depends on the previous). Direct execution or a single agent is faster — team overhead (create, monitor, shutdown) exceeds the benefit when there's no parallelism to exploit
